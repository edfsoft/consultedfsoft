...
class CcModel extends CI_Model
{
    ...
    {
        $details = "SELECT * FROM `patient_details` WHERE `deleteStatus` = '0' ORDER BY `patientId` DESC";
    ...
    public function updateProfileDetails()
    {
        ...
        if (!empty($_FILES['profilePhoto']['name'])) {
            ...
            $formattedId = str_pad($ccIdDb, 2, '0', STR_PAD_LEFT);
            $fileName = 'cc_profile_' . $formattedId . '.' . $ext;
            ...
            $this->db->where('id', $ccIdDb);
            $this->db->update('cc_details', ['ccPhoto' => $fileName]);
        }
        return true;
    }
